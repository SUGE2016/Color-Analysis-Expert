...
| PRJ-04 | 异步执行完整分析 | `POST /api/projects/{id}/run` 只传 steps，随后轮询项目和任务 | 首次返回 202 + taskId；任务最终 `success`；项目最终 `completed`；不得只凭 HTTP 202 判 PASS | PASS |
| PRJ-05 | 任务状态与进度 | 用 PRJ-04 的真实 taskId 调任务接口 | 可观察合法状态流转；progress 单调不下降且最终 100；params/result/startedAt/finishedAt 完整 | PASS |
| PRJ-06 | 空数据集运行前校验 | 对含空数据集的 draft 调 `/run`，运行前后查询任务数 | 返回 422 和稳定错误码；不创建 Task；不得返回 500；项目保持 draft | PASS |
...
| PRJ-11 | 向导持续保存 | 多次 `PUT /api/projects/{id}` 保存步骤、区域、图片方法配置，再 GET | 每次 200；后端配置与最后一次提交完全一致；刷新后可恢复 | PASS |
...
| PRJ-15 | 多数据集完整覆盖 | 对两个非空数据集运行项目，核对结果中的图片标识 | 两个数据集的全部输入图片均被处理；同名图片以 datasetId + imageId 区分 | PASS |
| PRJ-16 | 输出文件条件契约 | 分别执行未启用和启用边缘/出界分析的项目 | 基础三类 CSV 始终存在且非空；edgeColorCsv 仅在启用时必需；缺必需文件则任务 failed | PASS |
...
| PRJ-19 | 禁止客户端路径参数 | `/run` 请求中注入 modelImagePath/butterflyJsonPath/edgeJsonPath | 返回 400；不得读取或使用客户端指定的服务器路径 | PASS |
...
| PRJ-22 | 项目区域唯一来源与运行前校验 | 使用只有图片、无 regionsJson 的模板创建 draft；空 `config.regions` 调用 run；再保存步骤 3 合法区域后运行并检查工作区文件 | draft 创建成功；空区域返回 422 且不创建 Task；合法区域返回 202；`template-regions.json` 与项目 `config.regions` 一致 | PASS |
...
6. 启用边缘/出界分析时，`edgeColorCsv` 同样存在且合法；
